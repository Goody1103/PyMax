...
class WebSocketMixin(BaseTransport):
    ...
    async def _send_no_wait(
        ...
        if self._ws is None or not self.is_connected:
            self.logger.warning("Cannot send no-wait: not connected")
            return
        msg = self._make_message(opcode, payload, cmd)
        try:
            await self._ws.send(json.dumps(msg))
            self.logger.debug(
                "Sent fire-and-forget message opcode=%s cmd=%s seq=%s",
                opcode,
                cmd,
                msg["seq"],
            )
        except Exception as e:
            self.logger.error(f"Failed to send fire-and-forget message: {e}", exc_info=True)
